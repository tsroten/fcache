...
class TestFileCache(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertRaises(ValueError, fcache.cache.FileCache, self.appname, "z")
        self.assertRaises(ValueError, fcache.cache.FileCache, self.appname, "rz")
    ...
    def test_sync(self):
        self.cache["foo"] = b"value"
        self.assertFalse(
            os.path.exists(self.cache._key_to_filename(self.cache._encode_key("foo")))
        )
    ...
    def test_flag(self):
        self.cache.delete()
        self.assertRaises(
            FileNotFoundError, fcache.cache.FileCache, self.appname, flag="r"
        )
        self.assertRaises(
            FileNotFoundError, fcache.cache.FileCache, self.appname, flag="w"
        )
        ...
        self.assertRaises(UnsupportedOperation, self.cache.__setitem__, "foo", b"value")
    ...
    def test_subcache(self):
        ...
        subcache2 = fcache.cache.FileCache(self.appname + ".subcache1.subcache2")

        self.assertFalse(self.cache._is_subcache)
        self.assertTrue(subcache1._is_subcache)
        self.assertTrue(subcache2._is_subcache)
        self.assertEqual(
            dirname(subcache1.cache_dir), dirname(dirname(subcache2.cache_dir))
        )
...
class TestShelfCache(unittest.TestCase):
    def setUp(self):
        ...
        self.cache = shelve.Shelf(
            fcache.cache.FileCache(self.appname, flag="cs", serialize=False)
        )
